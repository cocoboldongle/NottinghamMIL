...
def main():
    ...
    for epoch in range(train_epoch):
        ...
        val_predictions, val_auc, val_acc, val_f1, val_loss = evaluate(milnet, criterion, val_loader, device, conf, 'Val')

        
        test_file_path = f'./test_predictions_epoch_{epoch+1}.csv'
        test_predictions, test_auc, test_acc, test_f1, test_loss = evaluate(
            milnet, criterion, test_loader, device, conf, 'Test',
        ...
        if test_f1 > best_state['test_f1']:
            ...
            save_model(conf=conf, model=milnet, optimizer=optimizer0, epoch=epoch, is_best=True)

    save_model(conf=conf, model=milnet, optimizer=optimizer0, epoch=epoch, is_last=True)
...
@torch.no_grad()
def evaluate(net, criterion, data_loader, device, conf, header, save_predictions=False, file_path='predictions.csv'):
    net.eval()
    ...
    for data in metric_logger.log_every(data_loader, 100, header):
        ...
        _, slide_preds, _ = net(image_patches, use_attention_mask=False)
    ...
    AUROC_metric = torchmetrics.AUROC(num_classes=conf.n_class, average='weighted', task='multiclass').to('cpu')
    F1_metric = torchmetrics.F1Score(num_classes=conf.n_class, average='weighted', task='multiclass').to('cpu')
    
    AUROC_metric(predictions, y_true)
    auroc = AUROC_metric.compute().item()

    F1_metric(predictions, y_true)
    ...
    if save_predictions:
        df = pd.DataFrame(predictions.numpy(), columns=['Score_Class_{}'.format(i) for i in range(predictions.shape[1])])
        df['True_Label'] = y_true.numpy()
        df['Name'] = names  
        df.to_csv(file_path, index_label='Index')

    return predictions, auroc, metric_logger.acc1.global_avg, f1_score, metric_logger.loss.global_avg
