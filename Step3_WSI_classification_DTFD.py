...
def get_arguments():
    ...
    parser.add_argument('--wandb_mode', default='online', choices=['offline', 'online', 'disabled'],
...
def evaluate(classifier, attention, dimReduction, UClassifier, criterion, data_loader, device, conf, header, distill='MaxMinS'):
    ...
    # 평균을 'weighted'로 설정하여 각 클래스의 샘플 수를 고려
    AUROC_metric = torchmetrics.AUROC(num_classes=conf.n_class, average='weighted', task='multiclass').to(device)
    AUROC_metric(y_pred, y_true)
    auroc = AUROC_metric.compute().item()

    # F1 점수도 마찬가지로 weighted로 설정
    F1_metric = torchmetrics.F1Score(num_classes=conf.n_class, average='weighted', task='multiclass').to(device)
